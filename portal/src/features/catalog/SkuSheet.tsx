  ...
  return (
    ...
    >
      ...
      >
        ...
        <h4 style={{ marginTop: 20, marginBottom: 8 }}>Ціни, грн</h4>
        <div className="tablewrap">
          <table>
            ...
            <tbody>
              {(['west', 'east'] as const).map((zone) => (
                <tr key={zone}>
                  <td>{PRICE_ZONE_LABEL[zone]}</td>
                  <td className="num">{money(sku.price[zone].net)}</td>
                  <td className="num">{money(sku.price[zone].gross)}</td>
                  <td className="num">
                    <b>{money(sku.price[zone].rrp)}</b>
                  </td>
                </tr>
              ))}
            </tbody>
