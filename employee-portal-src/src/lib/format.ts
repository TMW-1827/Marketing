...
/**
 * Гроші: завжди дві копійки й розряди через пробіл — «45 907,20».
 *
 * Копійки не ховаємо, навіть нульові: у прайсі колонка має читатись згори
 * вниз одним стовпчиком цифр. Розряди обов'язкові — сума замовлення без них
 * («45907,20») перечитується двічі, а її називають клієнту вголос.
 */
export function money(value: number): string {
  const [whole, frac] = value.toFixed(2).split('.')
  return `${whole.replace(GROUP, THIN)},${frac}`
}
