...
import { BoxIcon, HomeIcon, ListIcon, TagIcon } from './Icons'
          ...
          <div key={group}>
            ...
            {byGroup(group).map((section) => (
              ...
              >
                ...
                {/* Мітка «кому» — щоб новий працівник бачив, що з переліку
                    його, а що сусіда. «Усім» не позначаємо: це шум. */}
                {section.audience && section.audience !== 'all' && (
                  <i className="drawer__badge">
...
export function TabBar({ onOpenSections }: { onOpenSections: () => void }) {
  ...
  return (
    <nav className="tabbar" aria-label="Основна навігація">
      ...
      <button
        type="button"
        className={pathname === '/prices' ? 'is-active' : undefined}
      ...
      <button
        type="button"
        className={pathname === '/storage' ? 'is-active' : undefined}
